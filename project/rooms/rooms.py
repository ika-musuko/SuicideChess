...
class RoomManager:
    ...
    def new_room(self, players: list, mode: str, variant: str, status: str="waiting") -> (str, dict):
        # get the latest game number and use as an IDga
    ...
    def join_friend_game(self, user_id: str, room_id: str) -> (str, dict):
        db_query = self.pyre_db.child(self.game_branch).child(room_id).get()
        ...
        self.pyre_db.child(self.game_branch).child(room_id).set(requested_room)
